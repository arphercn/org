...
var formData = new FormData();
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data['code'] == 2000){
                        $('#pic').attr('src', '{{ asset("uploads") }}'+'/'+data['img']);
                        $('#thumb').val(data['img']);
                    }else{
                        $.gritter.add({
                            title: '操作消息！',
...
/**
    ...
    public function upload(Request $request)
    {
        ...
        if(!in_array($postfix, array('png','jpeg','jpg','gif'))){
            return response()->json(['code' => 2001,'img' => $fileName, 'error'=>'文件格式不对']);
        }

        if($file->getSize() > 1024*1024*2){
            return response()->json(['code' => 2002,'img' => $fileName, 'error'=>'文件太大']);
        }

        if($file->move($path, $fileName)){
            return response()->json(['code' => 2000,'img' => $fileName, 'error'=>'']);
        }
...
* 树
*** 最后应用版 从数组看依次取出10条id,在根据id取出相应的父节点数据，最后过滤10条里的重复数组
...
http://www.cnblogs.com/hxhbluestar/archive/2011/11/17/2252009.html
** 递归json to html
...
* css
<button type="button" id="btnStartModalTag" class="btn btn-success">新增标签</button>
button不设置type="button"默认在form里会跳转
